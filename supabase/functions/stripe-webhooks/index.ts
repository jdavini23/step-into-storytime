...
if (!supabaseUrl || !supabaseServiceRoleKey) {
  console.error('FATAL: Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY environment variable.');
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (!session.customer || !session.subscription || !session.client_reference_id) {
          console.error(`Error: Checkout session ${session.id} is missing essential data (customer, subscription, or client_reference_id).`);
          // Acknowledge receipt to Stripe but log internal error.
          return new Response(JSON.stringify({ received: true, error: 'Missing essential session data.' }), { status: 200 });
        }
        ...
        console.log(`Checkout session ${session.id} completed for user ${userId}. Attempting to upsert subscription ${stripeSubscriptionId}...`);
        ...
        try {
          ...
          if (!subscriptionData.plan_id) {
            console.warn(`Warning: Could not determine plan_id for subscription ${subscription.id}. Price data might be missing.`);
          }
          ...
          if (upsertError) {
            console.error(`Error upserting subscription ${subscription.id} for user ${userId} in Supabase:`, upsertError);
            // Log the error, but still return 200 to Stripe unless it's a critical failure scenario
          } else {
            console.log(`Successfully upserted subscription ${subscription.id} for user ${userId} in Supabase.`);
          }

        } catch (error: unknown) {
          // Catch errors from Stripe API call or DB upsert
          console.error(`Error processing checkout session ${session.id} (Stripe API or DB operation):`, error);
