  ...
  try {
    ...
    try {
      ...
      if (count === 0) {
        ...
        const freeTierSubscription: DbSubscription = {
          id: "free",
          user_id: user.id,
          status: "active",
          plan_id: "free",
        ...
        return NextResponse.json<DbSubscription>(freeTierSubscription);
      }
      ...
      if (subscription) {
        return NextResponse.json<DbSubscription>(subscription);
      }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate tier
    const validTiers: SubscriptionTier[] = ["free", "story_creator", "family"];
    ...
    try {
      ...
      const { error: profileError } = await supabase
        .from("profiles")
        .update({
          subscription_tier: tier === "story_creator"
            ? "premium"
            : tier === "family"
            ? "premium"
            : "basic",
