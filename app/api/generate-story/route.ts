...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.mainCharacter?.name) {
      return NextResponse.json({ error: "Missing main character name" }, { status: 400 })
    }

    if (!body.setting) {
      return NextResponse.json({ error: "Missing story setting" }, { status: 400 })
    }

    if (!body.theme) {
      return NextResponse.json({ error: "Missing story theme" }, { status: 400 })
    }
    ...
    const prompt = `
      ...
      Title: ${body.title}
      Main Character: ${body.mainCharacter.name}, who is ${body.mainCharacter.age || "young"} and is ${body.mainCharacter.traits?.join(", ") || "adventurous"}
    ...
    try {
      ...
      if (error.name === "AbortError") {
        return NextResponse.json({ error: "Story generation timed out. Please try again." }, { status: 408 })
      }
      throw error
    }
  } catch (error) {
