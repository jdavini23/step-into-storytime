...
export default function SubscriptionClient({
  ...
  const [plans] = useState<UISubscriptionPlan[]>(initialPlans);
    ...
    try {
      ...
      if (plan) {
        await createSubscription(
          plan.tier as 'free' | 'story_creator' | 'family'
        );
            ...
            {subscriptionState.subscription && (
              <div className="mt-4">
                ...
                <Link href="/subscription/manage">
                  <Button
                    variant="link"
                    className="text-violet-600 h-auto p-0 ml-2"
                  >
                    Manage Subscription
                  </Button>
                </Link>
              ...
              <div key={plan.id}>
                ...
                <Label
                  ...
                  className={`
                    ...
                    ${
                      plan.is_popular
                        ? 'ring-2 ring-violet-200 shadow-md'
                      ...
                      {plan.features.map(
                        (feature: PlanFeature, index: number) => (
                          <li key={index} className="flex items-center">
                            <Star
                              className="h-3.5 w-3.5 text-violet-600 mr-2 flex-shrink-0"
                              fill="#7c3aed"
                            />
          ...
          <div className="mt-10 flex justify-center">
            <Button
              ...
              disabled={
                !selectedPlan ||
                isLoading ||
                currentTier === plans.find((p) => p.id === selectedPlan)?.tier
              }
            ...
            >
              {isLoading ? (
                ...
                plans.find((p) => p.id === selectedPlan)?.tier ? (
                'Current Plan'
              ) : (
                'Subscribe Now'
              )}
