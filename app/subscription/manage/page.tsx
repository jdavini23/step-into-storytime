...
export default function ManageSubscriptionPage() {
  ...
  useEffect(() => {
    if (authState.isAuthenticated && !subscriptionState.isInitialized && !subscriptionState.isLoading) {
      fetchSubscription().catch(error => {
        console.error('[DEBUG] Error loading subscription:', error);
      });
    }
  }, [authState.isAuthenticated, subscriptionState.isInitialized, subscriptionState.isLoading, fetchSubscription]);

  useEffect(() => {
    if (subscriptionState.subscription && subscriptionState.availablePlans) {
      const product = subscriptionState.availablePlans.find(
        (plan) => plan.tier === subscriptionState.subscription?.subscription_plans.tier
      );
  ...
  if (subscriptionState.isLoading || authState.isLoading || !subscriptionState.isInitialized) {
    return (
                ...
                <CardContent className="space-y-6">
                  <div className="space-y-2">
                    ...
                    {currentProduct && currentPrice ? (
                      <>
                        <p>
                          You are currently subscribed to the <b>{currentProduct.name}</b> plan, which costs <b>{currentPrice.unit_amount / 100} {currentPrice.currency}</b> per {currentPrice.recurring.interval}.
                        </p>
                      </>
                    ) : (
