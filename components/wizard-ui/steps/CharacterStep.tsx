...
const TeddyBearSVG = () => (
  <svg aria-hidden="true" width="48" height="48" viewBox="0 0 48 48" fill="none" className="inline-block align-middle mr-2 -mt-1"><circle cx="24" cy="28" r="12" fill="#fde68a"/><circle cx="14" cy="18" r="5" fill="#fde68a"/><circle cx="34" cy="18" r="5" fill="#fde68a"/><ellipse cx="24" cy="34" rx="4" ry="2.5" fill="#fbbf24"/><ellipse cx="24" cy="28" rx="7" ry="6" fill="#fff"/><ellipse cx="24" cy="30.5" rx="2.5" ry="1.5" fill="#fbbf24"/><circle cx="20" cy="27" r="1" fill="#444"/><circle cx="28" cy="27" r="1" fill="#444"/><ellipse cx="24" cy="31.5" rx="1" ry="0.5" fill="#444"/></svg>
);

const genderOptions = [
  { value: 'boy', label: 'Boy', icon: <span role="img" aria-label="Boy">🧒</span> },
  { value: 'girl', label: 'Girl', icon: <span role="img" aria-label="Girl">👧</span> },
];

const traitOptions = [
  'Brave', 'Curious', 'Kind', 'Silly', 'Adventurous', 'Creative', 'Helpful', 'Gentle'
];
...
const CharacterStep: React.FC = () => {
  ...
  useEffect(() => {
    setCanGoNext(
      Boolean(watchedName && watchedGender && watchedTraits && watchedTraits.length > 0 && watchedAge && Number(watchedAge) >= 2 && Number(watchedAge) <= 12)
    );
  ...
  const toggleTrait = (trait: string) => {
    ...
    if (traits.includes(trait)) {
      setValue('traits', traits.filter((t) => t !== trait), { shouldValidate: true });
    } else {
  ...
  return (
    <form className="space-y-6" autoComplete="off">
      ...
      <p className="text-sm text-gray-500 mb-4">Let’s make it personal! Tell us about your child’s name, age, gender, and favorite traits.</p>
      <div className="space-y-3">
        <label className="block font-semibold">Child’s Name</label>
        <div className="flex items-center gap-2">
          <span className="text-2xl">📝</span>
          <div className="relative flex-1">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-pink-400 text-xl pointer-events-none select-none">✨</span>
            <input
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-full border-2 text-base font-semibold shadow-sm transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-indigo-400
                  ...
                  ${selected ? 'border-indigo-500 text-indigo-800 scale-105 shadow-md ring-2 ring-indigo-200' : 'border-gray-200 text-gray-700 hover:scale-105 hover:shadow-lg'}
                `}
              ...
              >
                {age}
                {selected && (
                  <span className="ml-1 animate-bounce text-yellow-400 select-none" aria-hidden="true">🎈</span>
                )}
              </button>
            );
          })}
        ...
        <div className="flex gap-3">
          {genderOptions.map((g) => (
            <button
              type="button"
              key={g.value}
              className={`flex flex-col items-center justify-center p-3 rounded-xl border-2 transition font-semibold text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 ${watchedGender === g.value ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 bg-white hover:border-indigo-300'}`}
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          {traitOptions.map((trait) => (
            <button
              type="button"
              key={trait}
              className={`px-3 py-1 rounded-full border-2 text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-indigo-400 ${watchedTraits.includes(trait) ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 bg-white hover:border-indigo-300'}`}
        ...
        <form className="flex gap-2 items-center mt-2" onSubmit={handleAddCustomTrait}>
          <input
            type="text"
            value={customTrait}
          ...
          >
            <svg aria-hidden="true" width="18" height="18" viewBox="0 0 20 20" fill="none"><path d="M10 4v12M4 10h12" stroke="#a21caf" strokeWidth="2" strokeLinecap="round"/></svg>
            Add
          </button>
        </form>
      </div>
