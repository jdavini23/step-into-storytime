...
export function StoryNarration({
  ...
  useEffect(() => {
    ...
    audio.addEventListener('timeupdate', () => {
      ...
      const currentIndex = paragraphTimestamps.current.findIndex(
        (timestamp, index) => {
          const nextTimestamp =
            paragraphTimestamps.current[index + 1] || Number.POSITIVE_INFINITY;
          return (
            audio.currentTime >= timestamp && audio.currentTime < nextTimestamp
          );
        }
      );
