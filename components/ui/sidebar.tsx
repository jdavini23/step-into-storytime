...
>(({ className, asChild, showOnHover = false, ...props }, ref) => {
  const Comp = asChild ? Slot : 'button';
...
interface SidebarMenuLinkProps extends React.ComponentPropsWithoutRef<'a'> {
  asChild?: boolean;
  isActive?: boolean;
  tooltip?: string | React.ComponentProps<typeof TooltipContent>;
  variant?: VariantProps<typeof sidebarMenuButtonVariants>['variant'];
...
>(({ className, ...props }, ref) => {
  return (
    <span
      ref={ref}
      className={cn(
        'ml-auto flex h-6 w-6 items-center justify-center rounded-full bg-muted text-sm font-medium',
        className
      )}
      {...props}
    />
...
>(({ className, children, ...props }, ref) => {
  return (
    <button
      ref={ref}
      className={cn(
        'flex w-full items-center justify-between rounded-md px-2 py-1 text-sm font-medium hover:bg-accent hover:text-accent-foreground',
        className
      )}
      {...props}
    >
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn(
        'flex items-center gap-2 rounded-md px-2 py-1 text-sm hover:bg-accent hover:text-accent-foreground',
        className
      )}
      {...props}
    />
