    ...
    if (authCookie) {
      try {
        ...
        if (error instanceof Error) {
          console.error("[DEBUG] Middleware - setSession failed:", {
          ...
          console.log("[DEBUG] Middleware - setSession success:", {
            userId: session?.user?.id,
            expiresAt: undefined, // Remove data reference since it's not in scope
            path: req.nextUrl.pathname,
            timestamp: new Date().toISOString(),
          });
        }
