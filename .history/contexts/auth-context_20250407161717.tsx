...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const updateAuthState = async (user: User | null) => {
      ...
      try {
        ...
        // Silent error in production, only log in development
        if (process.env.NODE_ENV === "development") {
          console.error(
            "Error updating auth state:",
    ...
    const {
      ...
      // Silently handle auth state changes without logging
  ...
  // Add detailed logging to inspect Supabase response data
  const login = useCallback(
    async (email: string, password: string) => {
      try {
        ...
        if (!data?.session) {
          console.error("[DEBUG] No session data received:", data);
          throw new Error("No session data received from authentication service");
        }

        console.log("[DEBUG] Session and user data:", data.session, data.user);

        dispatch({ type: "SET_USER", payload: data.session.user });
        console.log("[DEBUG] State updated with user:", data.session.user);
